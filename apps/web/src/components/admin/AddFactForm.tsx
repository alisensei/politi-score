...
import { SEVERITY_LABELS } from '@politi-score/types'
...
export default function AddFactForm({ politicianId, table, severities, slug }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    fact_check_org: '',
    conflict_type: 'financier',
    declared_hatvp: false,
    date_start: '',
    source_label: '',
    source_url: '',
    source_type: 'presse',
  ...
  const handleSubmit = async () => {
    ...
    } else if (table === 'opacity') {
      payload.description = form.description
      payload.date_constat = form.date_start || null
    } else if (table === 'harm') {
      payload.description = form.description
      payload.date_start = form.date_start || null
    } else if (table === 'speech_offenses') {
      payload.description = form.description
      payload.date_event = form.date_start || null
    }
  ...
  const inputClass =
    'w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-sm font-medium outline-none focus:border-gray-800 transition-colors bg-white'
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {table === 'conflicts' && (
              <div>
                ...
                >
                  {['financier', 'familial', 'professionnel', 'actionariat'].map((t) => (
            ...
            {(table === 'probity' || table === 'opacity' || table === 'harm' || table === 'speech_offenses') && (
              <div>
            ...
            {table === 'sincerity' ? (
              <>
                <div className="col-span-2">
                  ...
                  <textarea
                    className={inputClass}
                    rows={2}
                    value={form.statement_original}
                    onChange={(e) => set('statement_original', e.target.value)}
                    placeholder="Citation exacte..."
                  />
                </div>
                <div className="col-span-2">
                  ...
                  <textarea
                    className={inputClass}
                    rows={2}
                    value={form.statement_correction}
                    onChange={(e) => set('statement_correction', e.target.value)}
                    placeholder="La réalité des faits..."
                  />
                </div>
                <div>
                  ...
                  <input
                    className={inputClass}
                    value={form.fact_check_org}
                    onChange={(e) => set('fact_check_org', e.target.value)}
                    placeholder="AFP Factuel, Les Décodeurs..."
                  />
                </div>
              </>
            ) : (
              <div className="col-span-2">
                ...
                <textarea
                  ...
                  placeholder="Description factuelle détaillée..."
                />
              </div>
            )}
          </div>
          ...
          <div className="border-t border-gray-100 pt-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                >
                  {['presse', 'legal', 'officiel', 'hatvp', 'parquet', 'autre'].map((t) => (
                    <option key={t} value={t}>
                      {t}
                    </option>
                  ))}
